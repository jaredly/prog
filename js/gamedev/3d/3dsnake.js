...
function draw_matrix(matrix){
    ...
    for (var x=0;x<w;x++){
        ...
        var [nx,nz] = rot_around([x-w/2,z-d/2],[0,0],horiz);
        var [ny,nz] = rot_around([y-h/2,nz],[0,0],vert);
    ...
    for (var i=0;i<all.length;i++){
        var [z,x,y,c,a] = all[i];
            ...
            draw_circle(all[i][1]*size,all[i][2]*size,all[i][0]/5+2,20);
...
function load(){
    ...
    for (var z=0;z<d;z++){matrix.push([])
    for (var y=0;y<h;y++){matrix[z].push([])
    for (var x=0;x<w;x++){
        matrix[z][y].push(3);
    }}}

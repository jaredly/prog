...
var Three = Class([],{
    ...
    _3to2:function(self,x,y,z){
        var c = self.c;
        return [c[0] + self.perspective(x-c[0], z), c[1]+self.perspective(y-c[1], z)];
    },
    draw_circle:function(self,ctx,x,y,z,size,line){
        var p = self._3to2(self.c[0]+x,self.c[1]+y,self.c[2]+z);
        ctx.beginPath();
        ctx.arc(p[0],p[1],self.perspective(size,z),0,Math.PI*2,true);
        ctx.fill();
        if (line){
    ...
    if (state[4]){
        ...
        for (var i=0;i<lines.length;i++){
            ...
            for (var e=0;e<lines[i].length;e++){
                ...
                var [nx,nz] = rot_around([x-1.5,z-1],[0,0],d3.hrot);
                var [ny,nz] = rot_around([y-1.5,nz],[0,0],d3.vrot);
                var [na,nd] = rot_around([a-1.5,d-1],[0,0],d3.hrot);
                var [nb,nd] = rot_around([b-1.5,nd],[0,0],d3.vrot);
                var p1 = d3._3to2(d3.c[0]+nx*d3.size,d3.c[1]+ny*d3.size,nz/5+1);
                var p2 = d3._3to2(d3.c[0]+na*d3.size,d3.c[1]+nb*d3.size,nd/5+1);
                ctx.beginPath();
...
function draw_matrix(matrix){
    ...
    for (var i=0;i<all.length;i++){
        ...
        if (state[c] == -1)
            ...
            d3.draw_circle(ctx,all[i][1]*d3.size,all[i][2]*d3.size,(all[i][0]/5+1),6);
        }else{
            ctx.fillStyle = colors[c];
            ctx.globalAlpha = 1;
            if (a)ctx.globalAlpha = .1;
            d3.draw_circle(ctx,all[i][1]*d3.size,all[i][2]*d3.size,(all[i][0]/5+1),30,true);
        }
    }
    }
    if (frame==1 || frame==2){

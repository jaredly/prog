...
function max(x){return x.sort()[x.length];}
...
function loadFont(font,func){
  return sendRequest(font,
      function(x){func(_load(x.responseText))},
      null);
}
...
function _load(xmldata){ // parse the xmldata --> the svg font, return a list (letters)
  xmldata = xmldata.replace(/\n/g," ");
  var ascent = int( xmldata.match("<font-face.+?ascent=\"(.+?)\".+?>").slice(-1)[0] );
...
function Font(letters){
    ...
    that.draw = function(ctx,string,pos,size,color){
        ctx.fillStyle = color;
        draw_string(ctx, letters, size, string, pos);
    }
    that.render = function(string,size,color){
        var nc = document.createElement("canvas");
        var [w,h] = that.get_size(string,size);
        nc.width = w;
        nc.height = h;
        that.draw(nc.getContext("2d"),string,[0,0],size,color);
        return nc;
    }
    that.draw_center = function(ctx,string,[x,y],size,color){
        var [w,h] = that.get_size(string,size);
        that.draw(ctx,string,[x-w/2,y-h/2],size,color);
    }
    that.get_size = function(string,size){
        return string_size(letters,size,string);
    }
    return that;
}
var that = {};
that.Font = function(url,async){
    if (async){
        loadFont(url,function(x){async(Font(x));});
    }else{
        return Font(loadSync(url));
    }
}
return that;
})();